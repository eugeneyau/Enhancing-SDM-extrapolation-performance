...
#Truncation method
ttype <- "perc" # "perc" or "lat"
if (ttype == "perc") {
  filename1 <- "bmod424_trunc_VIF5_SOUTH_trun0perc" #0%
  filename2 <- "bmod424_trunc_VIF5_SOUTH_trun10perc" #10%
  filename3 <- "bmod424_trunc_VIF5_SOUTH_trun20perc" #20%
  filename4 <- "bmod424_trunc_VIF5_SOUTH_trun30perc" #30%
} else if (ttype == "lat") {
  filename1 <- "bmod424_trunc_VIF5_SOUTH_trun0perc" #Full
  filename2 <- "bmod424_RAWvspEMmean_trunc_VIF5_lat_5-36" #5-36
  filename3 <- "bmod424_RAWvspEMmean_trunc_VIF5_lat_10-36" #10-36
  filename4 <- "bmod424_RAWvspEMmean_trunc_VIF5_lat_15-36" #15-36
}

VirtualEM <- "EMmean"
EM <- "EMmean"

save.filename <- paste0("Trunlevel_VIF5_",ttype,"trun_",EM) # Name the df output file
...
if (ttype == "lat") {
  ...
  basepath_allvsp <- paste0("/lustre1/g/sbs_bonebrake/Eugene/SDM_BASE/results_",filename.base,"/")
  poss <- paste0(basepath_allvsp,species.list,BASEprojection,species.list,"_ensemble_TSSbin.tif")
  tf <- file.exists(poss)
  species.list <- species.list[tf]
  my_stack_base_allvsp <- terra::rast(paste0(basepath_allvsp,species.list,BASEprojection,species.list,"_ensemble_TSSbin.tif"))
  ...
  ### Stack clamping mask for uncertainty pattern
...
columns <- c("Species","Full.FP","Full.FP.perc","Full.FN","Full.FN.perc","Full.F","Full.F.perc",
             "TrunI.FP","TrunI.FP.perc","TrunI.FN","TrunI.FN.perc","TrunI.F","TrunI.F.perc",
             "TrunII.FP","TrunII.FP.perc","TrunII.FN","TrunII.FN.perc","TrunII.F","TrunII.F.perc",
             "TrunIII.FP","TrunIII.FP.perc","TrunIII.FN","TrunIII.FN.perc","TrunIII.F","TrunIII.F.perc")
...
# Automated calculation of accuracy for each species
...
lines(density(diff.perc.df$TrunI.FP.perc,n=10000),col="#364B9A")
lines(density(diff.perc.df$TrunII.FP.perc,n=10000),col="#4A7BB7")
lines(density(diff.perc.df$TrunIII.FP.perc,n=10000),col="#6EA6CD")

### False positive number
plot(density(diff.perc.df$Full.FP,n=10000),xlim = c(-1000,10000))
lines(density(diff.perc.df$TrunI.FP,n=10000),col="#364B9A")
lines(density(diff.perc.df$TrunII.FP,n=10000),col="#4A7BB7")
lines(density(diff.perc.df$TrunIII.FP,n=10000),col="#6EA6CD")


### False negative percentage
plot(density(diff.perc.df$Full.FN.perc,n=10000),xlim = c(-5,70))
lines(density(diff.perc.df$TrunI.FN.perc,n=10000),col="#364B9A")
lines(density(diff.perc.df$TrunII.FN.perc,n=10000),col="#4A7BB7")
lines(density(diff.perc.df$TrunIII.FN.perc,n=10000),col="#6EA6CD")

### False negative number
plot(density(diff.perc.df$Full.FN,n=10000),xlim = c(-1000,8000))
lines(density(diff.perc.df$TrunI.FN,n=10000),col="#364B9A")
lines(density(diff.perc.df$TrunII.FN,n=10000),col="#4A7BB7")
lines(density(diff.perc.df$TrunIII.FN,n=10000),col="#6EA6CD")


### Overall inaccuracy percentage
plot(density(diff.perc.df$Full.F.perc,n=10000),xlim = c(5,120))
lines(density(diff.perc.df$TrunI.F.perc,n=10000),col="#364B9A")
lines(density(diff.perc.df$TrunII.F.perc,n=10000),col="#4A7BB7")
lines(density(diff.perc.df$TrunIII.F.perc,n=10000),col="#6EA6CD")

### Overall inaccuracy number
plot(density(diff.perc.df$Full.F,n=10000),xlim = c(-1000,15000))
lines(density(diff.perc.df$TrunI.F,n=10000),col="#364B9A")
lines(density(diff.perc.df$TrunII.F,n=10000),col="#4A7BB7")
lines(density(diff.perc.df$TrunIII.F,n=10000),col="#6EA6CD")
...
# Mean
mean(diff.perc.df$Full.F)/totalcells
mean(diff.perc.df$TrunI.F)/totalcells
mean(diff.perc.df$TrunII.F)/totalcells
mean(diff.perc.df$TrunIII.F)/totalcells

# Median
median(diff.perc.df$Full.F)/totalcells
median(diff.perc.df$TrunI.F)/totalcells
median(diff.perc.df$TrunII.F)/totalcells
median(diff.perc.df$TrunIII.F)/totalcells
