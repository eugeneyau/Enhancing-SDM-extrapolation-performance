...
algoplot1 <- ggplot(ggplot.algo.df) + 
  stat_density(aes(x=f.F, colour=setup), geom="line", position="identity", linewidth=0.55) +
  stat_density(aes(x=t.F, colour=setup), geom="line", linetype=5, position="identity", linewidth=0.55) +
  ...
  labs(x = "Total false predictions", y = "Density", title = paste(algoname), fill="xyz") +
  scale_color_manual(values=c(A="#332288",B="#88CCEE")) + # "#33BBEE"
  scale_x_continuous(limits = c(-3000,22000), expand=c(0,0), breaks = seq(0,20000,5000)) +
  scale_y_continuous(limits = c(0,0.000225), expand=c(0,0), breaks = seq(0,0.0002,0.00005))
...
ggplot(ggplot.algo.df, aes(x = f.F, colour = combined_group, linetype = combined_group)) +  # Corrected aes mapping
  ...
  scale_color_manual(values = c("Tuned Full" = "#332288", "Untuned Full" = "#332288", "Tuned Truncated" = "#88CCEE", "Untuned Truncated" = "#88CCEE"),
                     ...
                     labels = c("Untuned algorithm, full data", "Tuned algorithm, full data", "Untuned algorithm, truncated data",  "Tuned algorithm, truncated data")) + 
