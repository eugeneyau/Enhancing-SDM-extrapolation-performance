...
#save.filename <- "Algo_VIF5_EMca_proj_TSSNA_trunNORTH" # Name the df output file
...
barplot.full.siteperc.df <- data.frame(matrix(nrow = 0, ncol = length(columns)))
colnames(barplot.full.siteperc.df) <- columns
barplot.full.rangeperc.df <- data.frame(matrix(nrow = 0, ncol = length(columns)))
colnames(barplot.full.rangeperc.df) <- columns
...
barplot.full.siteperc.df$Metric <- factor(barplot.full.siteperc.df$Metric, levels = c("fFP.siteperc", "fFN.siteperc"))
barplot.full.rangeperc.df$Metric <- factor(barplot.full.rangeperc.df$Metric, levels = c("fFP.rangeperc", "fFN.rangeperc"))
...
### Active: Median dodged (truncated data) ###
ggplot(data = barplot.siteperc.df) + 
  geom_bar(aes(x = Algo, y = Median, fill = Metric),stat = "identity", position = "dodge") +
  ...
  labs(x = "SDM algorithm", y = "Median false prediction percentage", fill=" ") +
  scale_fill_manual(values=c("#33BBEE","#882255"), 
                    labels=c("False positive      ","False negative      ")) #+
#scale_y_continuous(breaks = seq(0, 250, 50))

ggsave("algo_FPFNbar_VIF5_median_dodged_EMca.png", width = 30, height = 18.5, units = "cm", dpi = 800)


### Active: Median dodged (full data) ###
ggplot(data = barplot.full.siteperc.df) + 
  geom_bar(aes(x = Algo, y = Median, fill = Metric),stat = "identity", position = "dodge") +
  geom_hline(yintercept = 0, color = "dark grey", linewidth=0.5) +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
...
ggsave("algo_FPFNbar_VIF5_fullmedian_dodged_EMca.png", width = 30, height = 18.5, units = "cm", dpi = 800)
