...
#Model run name
datetime <- "bmod424_trunc_bio_phchndvi_VIF5"

#Virtual species location
vspdir <- "biomod424_BASE_VIF5"
...
current.climate <- terra::rast("filtered_current_climate.tif")
current.climate <- current.climate[[c("WorldClim_bio04","WorldClim_bio05","WorldClim_bio12",
                                      "WorldClim_bio14","WorldClim_bio15","NDVImean","Canopy_Height")]]
...
MyBiomodSF <- function(m) {
  try({
    ...
    if (nrow(df) > 19) { #GBM works with at least 20 records
      ...
      write.table("Failed", file.path(paste("/lustre1/g/sbs_bonebrake/Eugene/SDMout/",species.list.dir,"/",Species.list[m],"_Insufficient_Records_Failed.txt", sep="")))
    }
  })}
